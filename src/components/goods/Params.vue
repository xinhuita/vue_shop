<template>
    <div>
        ...
        <el-card>
            <!-- 头部的警告区 -->
            <el-alert title="注意：只允许为第三级分类设置相关参数! " type="warning" :closable="true" show-icon></el-alert>
            ...
            <el-row class="cat_opt">
                <el-col>
                    <span>选择商品分类：</span>
                    <!-- 选择商品分类的级联选择框 -->
                    <el-cascader expandTrigger="hover" v-model="selectedCateKeys" :options="catelist" :props="cateProps" @change="handleChange"></el-cascader>
                </el-col>
            </el-row>
        ...
        methods: {
            ...
            showInput(row) {
                row.inputVisible = true
                    //让文本框自动获得焦点
                    //$nextTick 方法的作用， 当页面上元素被重新渲染后才能执行回调函数中的代码
                this.$nextTick(_ => {
            ...
            //删除对应的参数可选项
            handleClose(i, row) {
                row.attr_vals.splice(i, 1) //会修改原数组
                this.saveAttrVals(row)
